...
describe('Persistent cache provider', async () => {
  ...
  const checkCache = async (provider: SessionCacheProvider | StorageCacheProvider, key: string): Promise<void> => {
    ...
    provider.setCache({key: key, returnType: CacheReturnType.Static}, '[]', 'test');
    ...
    const cacheModel: PersistentCacheModel<PersistentCacheOptions> = provider['storage'].getStorageItems(LocalStorageCacheKey).find(item => item.options.key === key);
    const containerModel: PersistentContainerModel = provider['storage'].getContainerItems(LocalStorageContainerKey).find(item => item.options.key === containerKey);
